...
export function useNFTMint(analysisId: string) {
    ...
    const { data: receipt, isLoading: isMinting } =
        useWaitForTransactionReceipt({
            hash: writeData,
            confirmations: 2,
        });
                        ...
                        try {
                            ...
                            if (receipt?.logs && receipt.logs.length > 0) {
                                ...
                                if (
                                    "topics" in lastLog &&
                                    lastLog.topics[0] ===
                                        "0xddf252ad1be2c89b69c2b068fc378daf11c3d7ac5e8b25e0e64e7737ea476ce3"
                                ) {
                                    const tokenIdFromLog = BigInt(
                                        lastLog.topics[3] || "0",
                                    ).toString();
