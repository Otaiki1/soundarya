  ...
  return (
    <div className="space-y-2.5">
      <div className="flex justify-between items-center text-[10px] tracking-[0.14em] uppercase">
        ...
        <span className="text-gold">{score}<span className="text-muted/60 ml-1">/{maxScore}</span></span>
      </div>
    ...
    <div className="surface-card p-6 sm:p-8 lg:p-10">
      ...
      <div className="mt-8 sm:mt-10 grid md:grid-cols-2 gap-4 sm:gap-5">
        <div className="p-5 sm:p-6 border border-white/8 bg-surface relative overflow-hidden group rounded-sm">
          ...
          <h4 className="font-serif text-xl sm:text-2xl text-gold-light mb-4 tracking-wide">Key Strengths</h4>
          <ul className="space-y-3">
            {analysis.strengths.map((strength, index) => (
              <li key={index} className="flex items-start text-xs leading-relaxed text-soft tracking-wide">
                <span className="text-gold mr-3 mt-1.5 text-[8px]">✦</span>
                {strength}
              </li>
            ))}
          </ul>
        ...
        <div className="p-5 sm:p-6 border border-gold/20 bg-gold/5 relative group transition-all hover:bg-gold/10 hover:border-gold/30 rounded-sm">
          <h4 className="font-serif text-xl sm:text-2xl text-gold mb-4 tracking-wide italic">Tailored Recommendation</h4>
          ...
          <div className="mt-5 pt-4 border-t border-gold/10 flex justify-between items-center opacity-60">
            <span className="text-[10px] tracking-[0.2em] uppercase text-gold">Priority Advice</span>
            <span className="text-lg text-gold animate-pulse">✦</span>
          </div>
        </div>
      </div>
    </div>
