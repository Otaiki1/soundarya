...
export function MintScoreModal({ isOpen, onClose, analysis }: MintScoreModalProps) {
  const { address, isConnected } = useAccount()
  const { data: balance } = useBalance({ address, chainId: 8453 })
  const [state, setState] = useState<MintState>('CONNECT')
  const { mint, isLoading, isSuccess, error, txHash, tokenId } = useNFTMint(analysis.id)
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface border border-gold/20 rounded-sm max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-surface border-b border-gold/10 p-6 flex justify-between items-center">
          <h1 className="font-serif text-2xl sm:text-3xl text-text">Mint Your Score Onchain</h1>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 sm:p-8">
          {state === 'CONNECT' && (
            <div className="space-y-6">
              <p className="text-soft text-sm sm:text-base leading-relaxed">
                Create a permanent, cryptographically-verified record of your score on Base. Cannot be faked or
                altered.
              </p>

              <div className="border border-gold/20 bg-gold/5 p-6 text-center rounded-sm">
                <p className="text-[10px] tracking-[0.2em] uppercase text-muted mb-3">Your Score</p>
                <div className={`font-serif text-6xl sm:text-7xl font-light mb-2 ${scoreColor}`}>
                  {analysis.overallScore.toFixed(1)}
                </div>
                <p className="text-xs text-muted tracking-wide">{analysis.category}</p>
              </div>

              <div className="border border-gold/10 bg-surface p-4 sm:p-5 rounded-sm">
                <p className="text-[10px] tracking-[0.2em] uppercase text-muted mb-2">Mint Price</p>
                <p className="font-serif text-xl text-gold">{MINT_PRICE} ETH</p>
                <p className="text-[11px] text-muted/70 mt-1">(~${MINT_PRICE_USD})</p>
              </div>
          ...
          {state === 'CONFIRM' && (
            <div className="space-y-6">
              ...
              <div className="border border-gold/20 bg-surface p-6 rounded-sm space-y-4">
                <div>
                  <p className="text-[10px] tracking-[0.2em] uppercase text-muted mb-3">Score Details</p>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted">Overall Score:</span>
                      <span className="text-gold font-serif">{analysis.overallScore.toFixed(1)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted">Percentile:</span>
                      <span className="text-gold">Top {analysis.percentile}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted">Category:</span>
                      <span className="text-gold">{analysis.category}</span>
                    </div>
                    <div className="mt-4 pt-4 border-t border-gold/10">
                      <p className="text-[10px] tracking-[0.2em] uppercase text-muted mb-3">Dimensions</p>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex justify-between">
                          <span className="text-muted">Symmetry:</span>
                          <span className="text-gold">{analysis.symmetryScore.toFixed(1)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Golden Ratio:</span>
                          <span className="text-gold">{analysis.goldenRatioScore.toFixed(1)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Bone Structure:</span>
                          <span className="text-gold">{analysis.boneStructureScore.toFixed(1)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Harmony:</span>
                          <span className="text-gold">{analysis.harmonyScore.toFixed(1)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Skin Quality:</span>
                          <span className="text-gold">{analysis.skinScore.toFixed(1)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted">Dimorphism:</span>
                          <span className="text-gold">{analysis.dimorphismScore.toFixed(1)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="border border-gold/20 bg-gold/5 p-4 rounded-sm space-y-2">
                <p className="text-[10px] tracking-[0.2em] uppercase text-muted font-medium">What Gets Stored</p>
                <ul className="space-y-1 text-xs text-soft">
                  <li className="flex items-start gap-2">
                    <span className="text-gold mt-0.5">✓</span>
                    <span>Permanent onchain record on Base</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-gold mt-0.5">✓</span>
                    <span>Cryptographic proof of your score</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-gold mt-0.5">✓</span>
              ...
              <div className="flex gap-3">
                ...
                >
                  {isLoading ? 'Minting...' : `Mint for ${MINT_PRICE} ETH`}
                </button>
                <button
                  onClick={handleClose}
                  disabled={isLoading}
          ...
          {state === 'MINTING' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              ...
              <div className="text-center space-y-2">
                <p className="font-serif text-xl text-text">Confirming transaction...</p>
                <p className="text-sm text-muted">Waiting for Base network confirmation</p>
              </div>
              {txHash && (
                <a
          ...
          {state === 'SUCCESS' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              ...
              <div className="text-center space-y-2">
                <p className="font-serif text-3xl sm:text-4xl text-text">Score Minted Successfully</p>
                {tokenId && <p className="text-gold font-serif text-lg">NFT #{tokenId}</p>}
              </div>

              <div className="border border-gold/20 bg-gold/5 p-4 rounded-sm w-full text-center">
                <p className="text-[10px] tracking-[0.2em] uppercase text-muted mb-2">Your Score</p>
                <p className={`font-serif text-4xl ${scoreColor}`}>{analysis.overallScore.toFixed(1)}</p>
              </div>
